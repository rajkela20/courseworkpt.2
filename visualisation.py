...
class AntColony:
    ...
    def _gen_paths(self):
        ...
        for _ in range(self.n_ants):
            path = self._gen_path(0)  # Start from city 0
...
def visualize_path(distances, path, iteration):
    G = nx.Graph()
...
aco = AntColony(distances, n_ants=15, n_iterations=200)
path, distance = aco.run()

print("Найденный путь:", path)
print("Длина пути:", distance)
# expected result is : ~5.3